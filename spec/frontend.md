...
- Каталог: карточки товаров (галерея — первое фото, затем видео товара
  ...
  Видео карточек грузятся сразу при рендере каталога —
  `<video preload="auto" controls src=...>`, чтобы быть готовыми к моменту
  перелистывания; нативные `controls` дают перемотку вперёд/назад, паузу и
