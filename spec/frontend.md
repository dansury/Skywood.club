...
- Каталог: карточки товаров (галерея — первое фото, затем видео товара
  ...
  свайп влево/вправо на тач-устройствах; перелистывание по горизонтальной
  позиции курсора при наведении на десктопе, возврат к первому фото при
  уходе курсора). Видео карточек грузятся лениво — `<video preload="none">`
  с `data-src`, источник проставляется `loadProductVideos()` после события
  `window.load`. Модалка товара (галерея — миниатюры, стрелки, свайп, ←/→).
