 ...
 * spec: spec/backend.md § Автообновление кода. Vendored from
 * site_yacloud_openrouter — fixes land upstream first, then get re-copied here.
 *
...
/** Switch + paths for this site: settings live in SQLite, creds in pull-config.php. */
function sw_autopull_opts(): array
{
    require_once __DIR__ . '/config.php';   // sw_root_dir()
    require_once __DIR__ . '/db.php';       // sw_setting(), sw_db_file()
    $root = sw_root_dir();
    return [
        'enabled'   => sw_setting('autopull_enabled', '0') === '1',
        'interval'  => (int)sw_setting('autopull_interval', '0'),
        'pull_url'  => sw_setting('autopull_url', ''),
        'root'      => $root,
        'state_dir' => dirname(sw_db_file()),
    ];
}

/**
 * Page hook — call it first thing in a PHP entry point, before any output.
 * The static pages of the site never reach PHP, so the check runs on the admin
 * pages and on API calls.
 */
function sw_autopull_run(): void
{
    AutoPull::run(sw_autopull_opts());
}
